...
import { RWBEventService } from './rwbevent.service';
...
export class RWBEventComponent implements OnInit {
  //#region form variables
  public goLog: GridOptions;
  readonly optionName: string = 'Rwb Event';
  frmRWBEvent: any;
  logData: RWBEvents[];
  lstEvent: any;
  lstAsset: any;
  ...
  lstDriver: any;
  errors: string[] = [];
  minDate = new Date(new Date().getDate() - 45);
  maxDate = new Date();
  //NewEventDateTime: any;
  stdTT: any;
  stdKMs: any;
  lastKMReading: any;
  distanceThreshold: any;
  leaseTypeId: any;
  lastDepartureDateTime: any;
  enablePartialDelivery: boolean = false;
  changeAsset: boolean = false;
  //EventTime: any;
  //EventDate: any;
  ...
  @ViewChild('eventId', { static: true }) eventId: MatSelect;
    //#endregion

  constructor(private router: Router, private formbulider: FormBuilder,
    private svcRWBEvent: RWBEventService, private svcToaster: agToasterService, private Enum: AgilityEnum,
  ...
  tbEdit() {
    ...
    if (this.enablePartialDelivery) {
      this.frmRWBEvent.controls.driverId1.enable();
      this.frmRWBEvent.controls.driverId2.enable();
    }
    else {
    ...
    this.frmRWBEvent.patchValue({ eventDate: new Date(), eventTime: formatDate(new Date(), 'HH:mm', 'en-US') });
  ...
  tbSave() {
    try {
      ...
      if (!this.frmRWBEvent.invalid) {
        var formData1 = this.frmRWBEvent.getRawValue();
        var formData: RWBEvent = this.frmRWBEvent.getRawValue();
        this.changeAsset = formData1.changeAsset;
  ...
  get(rwbNo: string) {
    ...
    this.svcWaitDlg.open({});
    try {
      this.svcRWBEvent.get(rwbNo).subscribe(
        rwbevent => {
          if (rwbevent) {
            this.frmRWBEvent.disable();
            ...
            if (this.enablePartialDelivery && rwbevent.data.lastEventDateTime != null) {
              if (rwbevent.data.emptryTrip) {
                ...
                this.frmRWBEvent.controls['lastDepartureDate'].setValue(formatDate(rwbevent.data.lastEventDateTime, 'dd-MMM-yyyy', 'en-US'));
                this.frmRWBEvent.controls['lastDepartureTime'].setValue(formatDate(rwbevent.data.lastEventDateTime, 'HH:mm', 'en-US'));
              }

              try {
                this.svcRWBEvent.getConsignees(rwbevent.data.rwbId).subscribe(
                  cn => {
                    if (cn) {
                      sessionStorage.removeItem("lstConsignee");
                ...
                this.svcRWBEvent.getShortage(rwbevent.data.rwbId).subscribe(
                  sh => {
                    if (sh) {
            ...
            try {
              this.svcRWBEvent.getRwbEvents(rwbNo).subscribe(
                log => {
  ...
  private loadLookup() {
    try {
      this.svcRWBEvent.getLookup().subscribe(
        data => {
  ...
  private loadRWBAsset(rwbId: number) {
    try {
      this.svcRWBEvent.getAssets(rwbId, 1).subscribe(
        data => {
  ...
  private validate(re: RWBEvent) {
    ...
    if (!re.outsourced) {
      ...
      if ((re.eventId == 2 || re.eventId == 4 || re.eventId == 7)) {
        ...
        if (!re.trailerId && !re.outsourced && this.AllowTrailer) {
        ...
        if (!re.driverId1 ) {
          this.errors.push('Driver 1 is mandatory');
        }

        if (this.IsMandatoryDriver2 && !re.driverId2) {
          this.errors.push('Driver 2 is mandatory');
        }
      ...
      if (!this.enablePartialDelivery && re.eventId == 8 || re.receiverCNIC ) {
      ...
      if (re.eventId == 8 && this.enablePartialDelivery) {
        var arrivalDateTime = formatDate(re.arrivalDate, 'dd-MMM-yyyy', 'en-US') + ' ' + re.arrivalTime;
        var deliveryDateTime = formatDate(re.eventDate, 'dd-MMM-yyyy', 'en-US') + ' ' + re.eventTime;
        var nextDeparture = formatDate(re.nextDepartureDate, 'dd-MMM-yyyy', 'en-US') + ' ' + re.nextDepartureTime;
        ...
        //}
        if (!re.emptryTrip && deliveryDateTime <= formatDate(this.lastDepartureDateTime, 'dd-MMM-yyyy HH:mm', 'en-US')) {
          this.errors.push('Delivery Date/Time is the time when vehicle arrives at selected consignee, it should be later than Last Departure Date/Time.');
        }
  ...
  onCosigneeChange() {
    ...
    this.frmRWBEvent.controls['lastDepartureTime'].setValue(formatDate(data[0].lastDepartureTime, 'HH:mm', 'en-US'));
  }
